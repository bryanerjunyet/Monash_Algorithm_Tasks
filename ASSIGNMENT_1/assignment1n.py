...
class MultiGraph(City):
    """
    Extended City class that models multiple time states as separate layers
    to handle repeated routes while maintaining optimal path finding.
    """
    def __init__(self, roads, stations):
        # Original initialization through parent class
        super().__init__(roads, stations)
        
        # Calculate maximum time needed (train loop duration)
        self.total_train_duration = sum(time for _, time in stations)
        self.num_layers = self.total_train_duration 
        
        # Expand locations to include time layers
        self.expanded_locations = []
        for time_layer in range(self.num_layers):
            for loc in range(self.total_location):
                new_loc_no = loc + time_layer * self.total_location
                new_location = Location(new_loc_no)
                new_location.original_loc = loc
                new_location.time_layer = time_layer
                self.expanded_locations.append(new_location)

        print("Expanded Locations:")
        for location in self.expanded_locations:
            print(f"Location No: {location.location_no}, Cost: {location.cost}, Time: {location.time}")

        # Connect roads across time layers
        for start, end, cost, time in roads:
            for time_layer in range(self.num_layers):
                new_time_layer = (time_layer + time) % self.num_layers
                # print("Time Layer:", time_layer, "New Time Layer:", new_time_layer)
                if new_time_layer < self.num_layers:
                    start_loc = start + time_layer * self.total_location
                    end_loc = end + new_time_layer * self.total_location
                    new_road = Road(start_loc, end_loc, cost, time)
                    self.expanded_locations[start_loc].add_road(new_road)
                    print("New Road:", new_road.start, new_road.end, new_road.cost, new_road.time)
    ...
    def dijkstra_search(self, start):
        """Modified Dijkstra to work with expanded locations"""
        # Initialize costs for all expanded locations
        location_cost = []
        for loc in self.expanded_locations:
            if loc.original_loc == start and loc.time_layer == 0:
                location_cost.append((0, loc.location_no))
                loc.cost = 0
                loc.time = 0
            else:
                location_cost.append((float('inf'), loc.location_no))
        ...
        while not location_heap.is_empty():
            # Choose location with lowest cost
            current_cost, location_no = location_heap.get_min()
            chosen_location = self.expanded_locations[location_no]
            current_time = chosen_location.time
            ...
            for road in chosen_location.outgoing_roads:     
                new_cost = current_cost + road.cost
                new_time = current_time + road.time
                next_location = self.expanded_locations[road.end]
                another_cost = next_location.cost
                another_time = next_location.time
...
class Location:
    ...
    def __init__(self, location_no) -> None:
        ...
        self.original_loc = -1  # original location number
        self.outgoing_roads = []
        ...
        self.time_layer = 0  # time layer for train stations
...
def intercept(roads, stations, start, friend_start):
    ...
    Uses MultiGraph to handle repeated routes while maintaining efficiency.
    """
    # Construction of city with time layers
    city = MultiGraph(roads, stations)
    city.dijkstra_search(start)
    
    # Track friend's starting position
    friend_position = -1
    for i in range(len(stations)):
    ...
    # Calculate accumulated train times
    acum_train_duration = [0] * len(stations)
    duration = 0
    position = friend_position
    for i in range(len(stations)):
        acum_train_duration[position] = duration
        duration += city.station_duration[position]
        position = (position + 1) % len(stations)
    
    intercept_route = None
    
    print("Expanded:")
    for location in city.expanded_locations:
        print(f"Location No: {location.location_no}, Cost: {location.cost}, Time: {location.time}")
  
    # Possible interceptions for each train station
    for station_no, time_travel in stations:
