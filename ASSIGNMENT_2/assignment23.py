...
def crowdedCampus(n, m, timePreferences, proposedClasses, minimumSatisfaction):
    ...
    for student in range(n):
        ...
        for class_id in range(m):
            if proposedClasses[class_id][0] in all_times and proposedClasses[class_id][0] not in set(timePreferences[student][:5]):
                network.add_edge(student, n + class_id, 1)
    ...
    for a in allocation:
        counts[a] += 1
    for j in range(m):
        min_cap = proposedClasses[j][1]
        max_cap = proposedClasses[j][2]
