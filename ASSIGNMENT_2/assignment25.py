...
def crowdedCampus(n, m, timePreferences, proposedClasses, minimumSatisfaction):
    ...
    for student in range(n):
        for class_id in range(m):
            ...
            if is_in_top5(time_slot, timePreferences[student]):
    ...
    for student in range(n):
        for class_id in range(m):
            time_slot = proposedClasses[class_id][0]
            if (is_in_preferences(time_slot, timePreferences[student]) and 
                not is_in_top5(time_slot, timePreferences[student])):
                network.add_edge(student, n + class_id, 1)
    ...
    for student in range(n):
        for edge in network.vertices[student].edges:
            # Remove isinstance check by ensuring we only have FlowEdge objects
