...
def crowdedCampus(n, m, timePreferences, proposedClasses, minimumSatisfaction):
    ...
    for i in range(n):
        ...
        for edge in student_node.edges:
            if edge.flow > 0 and TIME_PREF_START <= edge.v < TIME_PREF_START + 20:
                ...
                # Find which class this student was assigned to for this time slot
                for class_idx in time_to_classes[time_slot]:
                    class_node = network.vertices[CLASS_START + class_idx]
                    for class_edge in class_node.edges:
                        if class_edge.flow > 0 and (class_edge.v == SINK or class_edge.v == SUPER_SINK):
        ...
        if allocation[i] is None:
            ...
            for edge in student_node.edges:
                if edge.flow > 0 and edge.v == INTERMEDIATE1:
                    # Find which class has available capacity
                    for j in range(m):
                        class_node = network.vertices[CLASS_START + j]
                        for class_edge in class_node.edges:
                            if class_edge.flow > 0 and (class_edge.v == SINK or class_edge.v == SUPER_SINK):
                                allocation[i] = j
                                class_edge.flow -= 1
    ...
    # satisfied = 0
    # for i in range(n):
    #     class_idx = allocation[i]
    #     time_slot = proposedClasses[class_idx][0]
